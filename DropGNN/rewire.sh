...
run_script="python test_BREC.py --SEED=100 --hidden_units 16 --num_layers 10 --OUTPUT_DIM 16 --BATCH_SIZE 16 --LEARNING_RATE 0.0001 --WEIGHT_DECAY 1e-05 --EPOCH 100 --root results"
...
if [[ 0 == $1 ]]; then
    do_all "${run_script} --augmentation rewiring --rewire CGP --num_runs 1" "CGP"
    do_all "${run_script} --augmentation rewiring --rewire EGP --num_runs 1" "EGP"
    do_all "${run_script} --augmentation rewiring --rewire AE --num_runs 1" "AE"
    do_all "${run_script} --augmentation rewiring --rewire DE --num_runs 100" "DE"
else
    launch "${run_script} --augmentation rewiring --rewire CGP"
    launch "${run_script} --augmentation rewiring --rewire EGP"
fi
