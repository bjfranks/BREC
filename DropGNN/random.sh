...
run_script="python test_BREC.py --SEED=100 --hidden_units 16 --num_layers 10 --OUTPUT_DIM 16 --BATCH_SIZE 16 --LEARNING_RATE 0.0001 --WEIGHT_DECAY 1e-05 --EPOCH 100 --root results --added_dimensions 1"
...
if [[ 0 == $1 ]]; then
    do_all "${run_script} --augmentation random --random gaussian --num_runs 100" "gaussian"
    do_all "${run_script} --augmentation random --random RNI --num_runs 100" "RNI"
    do_all "${run_script} --augmentation random --random binary --num_runs 100" "binary"
    do_all "${run_script} --augmentation random --random IRNI --num_runs 1" "IRNI"
else
    launch "${run_script} --augmentation random"
fi
